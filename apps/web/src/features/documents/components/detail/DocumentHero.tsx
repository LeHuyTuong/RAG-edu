 ...
 * and backend-supported file actions.
 */
export function DocumentHero({ document }: Props): React.JSX.Element {
  const { download, open, isDownloading } = useDocumentFileActions(document);
    ...
    <section className="rounded-2xl border border-outline-variant bg-surface p-6">
      <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div className="flex shrink-0 flex-wrap gap-3">
          ...
          <Button disabled={isDownloading} onClick={download} variant="primary">
            <span className="flex items-center gap-2">
              <span className="material-symbols-outlined text-[18px]">
                download
              </span>
